...
void obrmgrp::setName(string name){
    if(name[0] == '"'){
...
Structs::SB obrmgrp::getSB(string path, int pos){
    Structs::SB sb;
    FILE * file = NULL;
...
Structs::TI obrmgrp::getInodo(string path, int pos){
    Structs::TI inodo_t;
    FILE * file = NULL;
    file = fopen(path.c_str(), "rb+");
    fseek(file, pos, SEEK_SET);
...
Structs::BC obrmgrp::getBC(string path, int pos){
    Structs::BC bc;
    FILE * file = NULL;
    file = fopen(path.c_str(), "rb+");
    fseek(file, pos, SEEK_SET);
...
Structs::BAR obrmgrp::getBAR(string path, int pos){
    Structs::BAR bar;
    FILE * file = NULL;
    file = fopen(path.c_str(), "rb+");
    fseek(file, pos, SEEK_SET);
...
list<string> obrmgrp::separar_carpetas(string path) {
...
list<string> obrmgrp::separar_comas(string path) {
    list<string> lista_carpetas;
    char ruta[path.length() + 1];
    strcpy(ruta, path.c_str());
    string nombre_carpeta;
...
void obrmgrp::exec(){
    ...
    if(!match){
        cout<<"\nNo existe el grupo o ya esta eliminado"<<endl;
        return;
    }
    string content;
