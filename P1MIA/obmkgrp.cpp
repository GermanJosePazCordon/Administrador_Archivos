...
void obmkgrp::setName(string name){
    if(name[0] == '"'){
...
Structs::SB obmkgrp::getSB(string path, int pos){
    Structs::SB sb;
    FILE * file = NULL;
...
Structs::TI obmkgrp::getInodo(string path, int pos){
    Structs::TI inodo_t;
    FILE * file = NULL;
    file = fopen(path.c_str(), "rb+");
    fseek(file, pos, SEEK_SET);
...
Structs::BC obmkgrp::getBC(string path, int pos){
    Structs::BC bc;
    FILE * file = NULL;
    file = fopen(path.c_str(), "rb+");
    fseek(file, pos, SEEK_SET);
...
Structs::BAR obmkgrp::getBAR(string path, int pos){
    Structs::BAR bar;
    FILE * file = NULL;
    file = fopen(path.c_str(), "rb+");
    fseek(file, pos, SEEK_SET);
...
list<string> obmkgrp::separar_comas(string path) {
    list<string> lista_carpetas;
    char ruta[path.length() + 1];
    strcpy(ruta, path.c_str());
    string nombre_carpeta;
...
void obmkgrp::exec(){
    ...
    if(this->name.length() > 10){
        cout<<"\nEl nombre debe de ser de 10 caracteres como maximo"<<endl;
        return;
    }
